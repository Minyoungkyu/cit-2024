<script lang="ts">
	...
	const clearedgameMapIds = playerLogList
		.filter((log) => log.detailInt! >= 1)
		.map((log) => log.gameMapId);
	const highestClearedgameMapId = Math.max(...gameMapIds);

	const difficultySelectorMsgs = [
		// 셀렉터 메시지
	...
	function findHighestStageStartId(highestClearedId: number): number {
		// 클리어한 최고 gameMapId 로 해금 스테이지 구하기 함수
	...
	function isOpen(stageId: number) {
		...
		return (
			clearedgameMapIds.includes(stageId) ||
			clearedgameMapIds.includes(stageNeedIds[stageStartIds.indexOf(stageId)])
		);
	}
	...
	onMount(() => {
		...
		function detectDeviceType() {
			// Todo: 디바이스 타입에 따라 editor focus 조절 예정
		...
		function adjustStageHighlighter(stageElement: Element) {
			// 하이라이터 위치 조절 함수
			const highlighter = document.getElementById('stageHighlighter');
		...
		const highestStageElement = document.querySelector(
			`div[data-gameMapId="${findHighestStageStartId(highestClearedgameMapId)}"]`
		);
...
<div
	class="content-container w-screen h-screen flex flex-col items-center justify-center bg-gray-500 overflow-hidden"
>
	<div
		class="background-container w-screen h-screen relative overflow-hidden"
		style="background-image:url('/background_1.png');background-position:center;background-size:cover;background-repeat:no-repeat;"
	>
		<div
			class="absolute top-[0] left-[0] w-[1226px] h-[523px] z-[60]"
		...
		<!--좌상단 백그라운드 레이어-->
		<div
			class="absolute top-[0] right-[0] w-[1044px] h-[445px] z-[60]"
		...
		<!--우상단 백그라운드 레이어-->
		<div
			class="absolute top-[0] right-[0] w-[386px] h-screen"
			style="transform:scale({scaleMultiplier});transform-origin:top right;"
		>
			<div
				class="absolute top-[0] right-[0] w-[386px] h-[1080px]"
				style="background-image:url('/img/map/ui_Gradation.png');"
			></div>
			<!-- 우측 백그라운드 레이어 -->
		</div>

		<div
			class="absolute top-[50%] right-[1%] w-[83px] h-[124px] cursor-pointer"
			on:click={() => rq.goTo('/game/2')}
			style="background-image:url('/img/map/btn_next.png');transform-origin:top right;transform:scale(0.67) scale({scaleMultiplier2});"
		></div>
		<!-- 다음 맵 버튼 -->

		<div
			class="w-[52px] h-[52px] absolute z-[97] right-[0] bottom-0 cursor-pointer"
			on:click={() => {
				myAudio.paused ? myAudio.play() : myAudio.pause();
				muted = !muted;
			}}
			style="background-image:url('/img/inGame/btn_Volume_{muted
				? 'mute'
				: 'on'}.png');transform:scale({scaleMultiplier});transform-origin:bottom right;"
		></div>

		<div
			class="flex flex-col absolute top-[2%] left-[2%] z-[60]"
			style="transform-origin:top left;transform:scale({scaleMultiplier2});pointer-events:none;"
		>
			<!-- 좌상단 -->
			<div
				class="flex flex-row items-end gap-5 h-[160px]"
				style="transform:scale(0.67) rotateZ(3deg) rotateY(5deg);transform-origin:left;"
			>
				<div
					class="{topMenuArray[0] ? '' : 'btn_stage'} cursor-pointer"
					style="background-image:{topMenuArray[0]
						? 'url("/img/map/btn_stage_off.png");width:160px;height:160px;'
						: 'url("/img/map/btn_stage_on.png");width:134px;height:134px;'}
                    transform:scale(1);background-repeat:no-repeat;background-size:contain;pointer-events:auto;"
					on:click={() => onClickTopMenu(0)}
				></div>
				<div
					class="{topMenuArray[1] ? '' : 'btn_shop'} cursor-pointer"
					style="background-image:{topMenuArray[1]
						? 'url("/img/map/btn_shop_off.png");width:160px;height:160px;'
						: 'url("/img/map/btn_shop_on.png");width:134px;height:134px;'}
                    transform:scale(1);background-repeat:no-repeat;background-size:contain;pointer-events:auto;"
					on:click={() => onClickTopMenu(1)}
				></div>
				<div
					class="{topMenuArray[2] ? '' : 'btn_codebook'} cursor-pointer"
					style="background-image:{topMenuArray[2]
						? 'url("/img/map/btn_coodbook_off.png");width:160px;height:160px;'
						: 'url("/img/map/btn_coodbook_on.png");width:134px;height:134px;'}
                    transform:scale(1);background-repeat:no-repeat;background-size:contain;pointer-events:auto;"
					on:click={() => onClickTopMenu(2)}
				></div>
				<div
					class="{topMenuArray[3] ? '' : 'btn_challenge'} cursor-pointer"
					style="background-image:{topMenuArray[3]
						? 'url("/img/map/btn_challenge_off.png");width:160px;height:160px;'
						: 'url("/img/map/btn_challenge_on.png");width:134px;height:134px;'}
                    transform:scale(1);background-repeat:no-repeat;background-size:contain;pointer-events:auto;"
					on:click={() => onClickTopMenu(3)}
				></div>
				<div
					class="{topMenuArray[4] ? '' : 'btn_rank'} cursor-pointer"
					style="background-image:{topMenuArray[4]
						? 'url("/img/map/btn_ranking_off.png");width:160px;height:160px;'
						: 'url("/img/map/btn_ranking_on.png");width:134px;height:134px;'}
                    transform:scale(1);background-repeat:no-repeat;background-size:contain;pointer-events:auto;"
					on:click={() => onClickTopMenu(4)}
				></div>
			</div>
			<div
				class="test font-bold text-white text-[50px] mt-2"
				style="text-shadow:-5px 5px black;pointer-events:none;"
			>
				{topMenuArrayText[currentMenuIndex]}
			</div>
		</div>

		{#if rq.member.authorities.length >= 2}
			<div
				class="btn_adm w-[330px] h-[74px] absolute right-[1%] top-[2%] z-[60] text-white text-[30px] font-bold flex justif-center items-center cursor-pointer"
		...
		<div
			class=" flex flex-col items-end absolute top-[4%] right-[0] z-[60]"
			style="transform-origin:top right;transform:scale({scaleMultiplier2});pointer-events:none;"
		>
			<!-- 우상단 -->
			<div
				class="flex flex-row gap-3 mr-4 h-[160px] items-end"
				style="transform-origin:right;transform:scale(0.67);"
			>
				<div
					class="w-[506px] h-[134px] {topMenuArray[5] ? '' : 'btn_profile'} cursor-pointer"
					style="background-image:{topMenuArray[5]
						? 'url("/img/map/ui_user_inf_on.png");transform:scale(1.05);transform-origin:bottom right;'
						: 'url("/img/map/ui_user_inf_off.png");'}
                    pointer-events:auto;"
					on:click={() => onClickTopMenu(5)}
				>
					<div
						class="text-white text-[40px] font-bold h-full flex flex-row items-center justify-between mr-4 italic"
					>
				...
				<div
					class="{topMenuArray[6] ? '' : 'btn_setting'} cursor-pointer"
					style="background-image:{topMenuArray[6]
						? 'url("/img/map/btn_settomg_on.png");width:160px;height:160px;'
						: 'url("/img/map/btn_settomg_off.png");width:134px;height:134px;'}
                transform:scale(1);background-repeat:no-repeat;background-size:contain;pointer-events:auto;"
					on:click={() => onClickTopMenu(6)}
				></div>
				<!-- <div class="w-[134px] h-[134px] btn_setting cursor-pointer" style="background-image:url('/img/map/btn_settomg_off.png');"></div> -->
				<div
					class="w-[134px] h-[134px] btn_logout cursor-pointer"
			...
			{#if topMenuArray[0]}
				<div
					class=" font-bold relative text-white text-[25px] text-center w-[510px] h-[216px] top-[-25px]"
				...
				>
					<div
						class="absolute left-[53%] top-[15%] text-[50px] w-[230px] text-right"
						style="text-shadow:2px 8px black;"
					>
		...
		{#if topMenuArray[1]}
			<div
				class="h-full absolute flex items-center justify-center z-[58]"
				style="background-image:url('/img/shop/background_menu.jpg');background-size:cover;width:{widthValue}px;"
			></div>
			{#if $shopGemsModalOpen}
				<div
					class="h-full w-full absolute flex items-center justify-center bg-black bg-opacity-50 z-[99]"
				>
					<div
					...
					>
						...
						>
							<div class=" ml-[100px] flex flex-col h-full items-center whitespace-nowrap">
								<div class="text-[150px] mt-[20px]">보석 부족</div>
								<div
								...
								<div class="flex flex-row w-full justify-center gap-12 text-[100px] mt-[30px]">
									<div
										class="w-[299px] h-[102px] text-gray text-[40px] font-bold italic text-center leading-[105px] cursor-pointer"
										style="background-image:url('/img/inGame/btn_action4.png');transform:scale(2)"
										on:click={() => shopGemsModalOpen.update((n) => false)}
									>
										확인
									</div>
								</div>
			...
			>
				<Shop {scaleMultiplier} resolution={adjustResolution} />
			</div>
		{/if}
		...
		{#if topMenuArray[2]}
			...
			>
				<Encyclopedia
					{scaleMultiplier}
					resolution={adjustResolution}
					closeFc={onClickTopMenuWithZero}
				/>
			</div>
		{/if}
		...
		{#if topMenuArray[4]}
			...
			<div
				...
				style="width:{widthValue}px;pointer-events:none;"
			>
				<Ranking {scaleMultiplier} resolution={adjustResolution} />
			</div>
		{/if}
		...
		{#if topMenuArray[5]}
			...
			<div
				...
				style="width:{widthValue}px;pointer-events:none;"
			>
				<Profile {scaleMultiplier} resolution={adjustResolution} />
			</div>
		{/if}
		...
		{#if topMenuArray[6]}
			...
			<div
				...
				style="width:{widthValue}px;pointer-events:none;"
			>
				<Setting {scaleMultiplier} resolution={adjustResolution} closeFc={onClickTopMenuWithZero} />
			</div>
		{/if}

		{#if showLogoutModal}
			<div
				class="h-full absolute flex items-center justify-center bg-black bg-opacity-50 z-[99]"
			...
			>
				...
				>
					...
					>
						<div class=" ml-[100px] flex flex-col h-full items-center whitespace-nowrap">
							<div class="text-[150px] mt-[20px]">로그아웃</div>
							<div
								class="absolute w-full h-[19px] top-[260px] left-[62px]"
								style="background-image:url('/img/inventory/ui_itme_window3.png');background-repeat:no-repeat;transform:scale(2.8);transform-origin:left;"
							></div>
							...
							<div class="flex flex-row w-full justify-around gap-12 text-[100px] mt-[30px]">
								...
								<div class="hovering text-red-500" on:click={() => (showLogoutModal = false)}>
									취소
								</div>
							</div>
						</div>
		...
		{#if rq.member.authorities.length < 2}
			<div
				id="stageHighlighter"
				class=" stage-highlighter absolute z-[10] {animationStart
					? 'animatedHighlighter'
					: 'invisible'}"
				style="width:{185 * scaleMultiplier2}px;height:{161 *
					scaleMultiplier2}px;background-image:url('/img/map/ui_aim.png');background-size:contain;pointer-events:none;background-repeat:no-repeat;bottom:{highlighterBottom -
					3}%;left:{highlighterLeft - 2.5}%;"
			></div>
		{/if}

		<!-- <div class="btn absolute bottom-[8%] left-[4%]" data-gameMapId="1" on:click={() => toggleDropdown(0)}>튜토리얼(열림)</div> -->
		<div
			class="stage_btn absolute w-[406px] h-[219px] bottom-[8%] left-[4%] cursor-pointer"
			data-gameMapId="1"
			on:click={() => toggleDropdown(0)}
			style="background-image: url(/img/map/ui_stage_{clearedgameMapIds.includes(2)
				? isDropdownOpen[0]
					? '3'
					: '2'
				: isDropdownOpen[0]
					? '3'
					: '1'}.png); transform:scale(0.67) scale({scaleMultiplier2});transform-origin:bottom left;"
		>
			<div
				class="stage-text absolute right-[1%] top-[-13px] text-[55px] text-white font-bold"
				style=""
			>
				튜토리얼
			</div>
			<div
				class="stage-text inE absolute right-[14%] top-[33%] text-[25px] text-white italic"
				style=""
			>
				TUTORIAL
			</div>
		...
		{#if isDropdownOpen[0]}
			...
			>
				<TutorialSelector {activeTransitionAnimation} />
			</div>
		{/if}
		{#if isOpen(3) || isUnLock(3) || isAdmin()}
			<!--step 의 easy, 1레벨 맵 아이디-->
			<div
				class="stage_btn absolute w-[406px] h-[219px] bottom-[40%] left-[20%] cursor-pointer"
				on:click={() => toggleDropdown(1)}
				data-gameMapId="3"
				style="background-image: url(/img/map/ui_stage_{clearedgameMapIds.some((value) =>
					[5, 8, 11].includes(value)
				)
					? isDropdownOpen[1]
						? '3'
						: '2'
			...
			{#if isDropdownOpen[1]}
				...
				<DifficultySelector
					...
					stepsLevelCount={3}
					{playerLogList}
					difficultySelectorMsg={difficultySelectorMsgs[0]}
					difficultySelectorName={difficultySelectorNames[0]}
		...
		{:else}
			...
			>
				<div
					class="stage-text absolute right-[7%] top-[-13px] text-[55px] text-gray-400 font-bold"
					style=""
				>
					<i class="fa-solid fa-lock text-[30px] mr-4"></i>1 - 1
				</div>
				<div
					class="stage-text inE absolute right-[7%] top-[33%] text-[25px] text-gray-400 italic"
					style=""
				>
					ONE - ONE
				</div>
		...
		{#if isOpen(12) || isUnLock(12) || isAdmin()}
			...
			<div
				class="stage_btn absolute w-[406px] h-[219px] bottom-[10%] left-[35%] cursor-pointer"
				on:click={() => toggleDropdown(2)}
				data-gameMapId="12"
				style="background-image: url(/img/map/ui_stage_{clearedgameMapIds.some((value) =>
					[14, 17, 20].includes(value)
				)
					? isDropdownOpen[2]
						? '3'
						: '2'
			...
			>
				<div
					class="stage-text absolute right-[7%] top-[-13px] text-[55px] text-white font-bold"
					style=""
				>
					1 - 2
				</div>
			...
			{#if isDropdownOpen[2]}
				<DifficultySelector
					...
					stepsLevelCount={3}
					{playerLogList}
					difficultySelectorMsg={difficultySelectorMsgs[1]}
					difficultySelectorName={difficultySelectorNames[1]}
					{activeTransitionAnimation}
				/>
			{/if}
		{:else}
			...
			>
				<div
					...
					style=""
				>
					<i class="fa-solid fa-lock text-[30px] mr-4"></i>1 - 2
				</div>
				<div
					class="stage-text inE absolute right-[14%] top-[33%] text-[25px] text-gray-400 italic"
					style=""
				>
					ONE - TWO
				</div>
		...
		{#if isOpen(21) || isUnLock(21) || isAdmin()}
			<div
				class="stage_btn absolute w-[406px] h-[219px] bottom-[45%] left-[52%] cursor-pointer"
				on:click={() => toggleDropdown(3)}
				data-gameMapId="21"
				style="background-image: url(/img/map/ui_stage_{clearedgameMapIds.some((value) =>
					[23, 26, 29].includes(value)
				)
					? isDropdownOpen[3]
						? '3'
						: '2'
			...
			{#if isDropdownOpen[3]}
				<DifficultySelector
					...
					stepsLevelCount={3}
					{playerLogList}
					difficultySelectorMsg={difficultySelectorMsgs[2]}
					difficultySelectorName={difficultySelectorNames[2]}
					{activeTransitionAnimation}
				/>
			{/if}
		{:else}
			...
			>
				<div
					...
					style=""
				>
					<i class="fa-solid fa-lock text-[30px] mr-4"></i>1 - 3
				</div>
				<div
					class="stage-text inE absolute right-[14%] top-[33%] text-[25px] text-gray-400 italic"
					style=""
		...
		{#if isOpen(30) || isUnLock(30) || isAdmin()}
			<div
				class="stage_btn absolute w-[406px] h-[219px] bottom-[65%] left-[52%] cursor-pointer"
				on:click={() => toggleDropdown(4)}
				data-gameMapId="30"
				style="background-image: url(/img/map/ui_stage_{clearedgameMapIds.includes(30)
					? isDropdownOpen[4]
						? '3'
						: '2'
					: isDropdownOpen[4]
			...
			{#if isDropdownOpen[4]}
				<div
					...
					style="transform-origin:top right;transform:scale({scaleMultiplier})"
				>
					<MiniGame1Selector {activeTransitionAnimation} />
				</div>
			{/if}
		{:else}
			<div
			...
			>
				<div
					...
					style=""
				>
					<i class="fa-solid fa-lock text-[30px] mr-4"></i>1 - 4
				</div>
				<div
					class="stage-text inE absolute right-[14%] top-[33%] text-[25px] text-gray-400 italic"
					style=""
