<script lang="ts">
	...
	const { data } = $props<{
		data: { gameMapDto: components['schemas']['GameMapDto']; guideItems: string[] };
	}>();
	...
	const killCount = stageObject.stage.init_item_list.filter(
		(item: any) => item.type.includes('monster') || item.type.includes('boss')
	).length;
	...
	let clearGoalColorArray = $state(
		Array.from({ length: clearGoalList.length }, () => 'rgb(64 226 255)')
	);
	...
	function editorSetVal() {
		if (editor.getValue() === '') {
			editor.setValue(
				localStorage.getItem(rq.member.username + '_' + gameMapDto.id) || gameMapDto.editorMessage,
				1
			);
		}
	}

	const customCompletions = gameMapDto.editorAutoComplete
		.split(',')
	...
	async function executePython(): Promise<void> {
		...
		if (gameMapDto.difficulty != '0') {
			baseScore =
				gameMapDto.difficulty === 'Easy' ? 100 : gameMapDto.difficulty === 'Normal' ? 200 : 500;
			playerScore = calculateBonus(gameMapDto.maxBonusCriteria, lineCounter);
		}

		let lineLimit = stageObject.stage.goal_list.filter((goal: any) => goal.goal === 'line')[0]
			?.count;
		...
		framePerSecond = 60;
		(window as any).SendStreamData?.(
			wrappedData,
			baseScore,
			playerScore,
			gameMapDto.maxBonusCriteria
		); // Todo: add checkLineCountforCocos in parameter
	...
	onMount(() => {
		...
		document.addEventListener('DOMContentLoaded', () => {
			...
			Object.defineProperty(body.style, 'width', {
				set: function () {
					this.setProperty('width', originalWidth, 'important');
				}
			});

			Object.defineProperty(body.style, 'height', {
				set: function () {
					this.setProperty('height', originalHeight, 'important');
				}
			});
		});
	...
	function updateHighlight(HilightRow: any) {
		// Todo: svelte 반응성으로 호출되도록 HighlightRow 변수하나 파서 반응성 걸기
	...
	function updateErrorHighlight(HilightRow: any) {
		// Todo: svelte 반응성으로 호출되도록 HighlightRow 변수하나 파서 반응성 걸기
		const Range = ace.require('ace/range').Range;
		const session = editor.getSession();
		session.removeMarker(markerId);
	...
	function updateClearGoal(frame: any, lineCounter: any) {
		for (let i = 0; i < clearGoalList.length; i++) {
			if (clearGoalList[i].includes('목표지점')) {
				...
				let items = stageObject.stage.init_item_list.filter(
					(goal: any) =>
						goal.type === 'liquid_fuel' ||
						goal.type === 'solid_propellant' ||
						goal.type === 'engines'
				);
				let count = 0;
	...
	function appendCodeToEditor(value: string) {
		...
		let newPosition = {
			...
			column: addedLines
				...
				: cursorPosition.column + value.length
		};
...
<div
	class="content-container flex flex-col items-center justify-center overflow-hidden bg-gray-700"
>
	{#if showResetConfirm}
		<div
			class="h-full w-screen absolute flex items-center justify-center bg-black bg-opacity-50 z-[99] overflow-hidden"
			style=""
		>
			<div
				class="flex justify-center items-center z-[90]"
				style="transform:scale(0.4) scale({scaleMultiplier});"
			>
				<div
					class="w-[80px] h-[904px]"
					style="background-image:url('/img/inventory/ui_popup_start.jpg');"
				></div>
				<div
					class="w-[1200px] h-[904px] text-white font-[900] text-[50px] flex flex-col items-center justify-around whitespace-nowrap"
					style="background-image:url('/img/inventory/ui_popup_middle.jpg');"
				>
					<div class=" ml-[100px] flex flex-col h-full items-center whitespace-nowrap z-[99]">
						<div class="text-[140px] mt-[24px]">에디터 초기화</div>
						<div
							class="absolute w-full h-[19px] top-[260px] left-[62px]"
							style="background-image:url('/img/inventory/ui_itme_window3.png');background-repeat:no-repeat;transform:scale(2.8);transform-origin:left;"
						></div>
						...
						<div class="flex flex-row w-full justify-around gap-12 text-[100px] mt-[30px]">
							<div
								class="hover:scale-[1.2] cursor-pointer"
								on:click={() => {
									editor.setValue(gameMapDto.editorMessage, 1);
									editor.focus();
							...
							<div
								class="hover:scale-[1.2] cursor-pointer text-red-500"
				...
				<div
					class="w-[228px] h-[904px] z-[98]"
					style="background-image:url('/img/inventory/ui_popup_end.png');background-repeat:no-repeat;"
				></div>
				<div
					class="absolute bg-gray-900 w-[90%] h-full z-[-1]"
	...
	{#if openSetting}
		...
		<div
			class="h-full mt-[-10%] absolute flex items-center justify-center z-[61]"
			style="width:{widthMultiplier}px;pointer-events:none;"
		>
			<Setting {scaleMultiplier} resolution={adjustResolution} closeFc={closeSetting} />
		</div>
	{/if}
	...
	{#if openEncyclopedia}
		<div class="w-screen h-screen absolute bg-black opacity-50 z-[60]" style=""></div>
		<div
			class="h-full mt-[-10%] absolute flex items-center justify-center z-[61] text-left"
			style="width:{widthMultiplier}px;pointer-events:none;"
		>
			<Encyclopedia {scaleMultiplier} resolution={adjustResolution} closeFc={closeEncyclopedia} />
		</div>
	{/if}
	...
	<div class="background-container w-screen h-screen relative flex flex-row overflow-hidden">
		{#if showBtnGuide}
			<div
				class="absolute guide-container h-full flex items-center justify-center z-[100]"
				style="width:{widthMultiplier}px;"
			>
				<div
					class="flex justify-center items-center z-[90]"
					style="transform:scale(0.5) scale({scaleMultiplier});"
				>
					<div
						class="w-[80px] h-[904px]"
						style="background-image:url('/img/inventory/ui_popup_start.jpg');"
					></div>
					<div
						class="w-full h-[904px] text-white font-[900] text-[50px] flex flex-col items-center justify-around whitespace-nowrap"
						style="background-image:url('/img/inventory/ui_popup_middle.jpg');"
					>
						...
						<div
							class="flex w-[1300px] h-[90%] justify-center items-center"
							style="white-space:pre-wrap;font-size:3.0rem;"
						>
							{guideItems[currentGuideIndex]}
						</div>
						<div class="flex flex-row w-full justify-center gap-20 mb-[80px]">
							<div
								class="w-[46px] h-[46px] cursor-pointer {currentGuideIndex === 0
									? 'invisible'
									: ''}"
								style="background-image:url('/img/inGame/btn_next5.png');transform:scale(1.5);"
								on:click={() => guideToPrev()}
							></div>
							<div
								class="btnGUideNext w-[46px] h-[46px] cursor-pointer {currentGuideIndex ===
								btnGuideArray.length - 1
									? 'invisible'
									: ''}"
								style="background-image:url('/img/inGame/btn_next6.png');transform:scale(1.5);"
								on:click={() => guideToNext()}
							></div>
						</div>
					</div>
					<div
						class="w-[80px] h-[904px]"
						style="background-image:url('/img/inventory/ui_popup_start.jpg');transform:scaleX(-1)"
					></div>
					<div
						class="absolute bg-gray-900 w-full h-full z-[-1]"
						style="clip-path:polygon(10% 0, 100% 0, 100% 90%, 90% 100%, 0 100%, 0 10%);"
					></div>
					{#if currentGuideIndex === btnGuideArray.length - 1}
						<div
							class="absolute w-[46px] h-[46px] top-[60px] right-[60px] cursor-pointer btnGUideNext"
		...
		{#if showClearPopup}
			<div
				class="absolute top-[50%] left-[50%] w-[1172px] h-[871px] z-[80]"
				style="background-image:url('/img/inGame/clearPop/ui_popup_clear_background.png');transform:translate(-50%, -50%) scale({scaleMultiplier -
					scaleMultiplier * 0.15});"
			>
				<div
					class="text-[43px] font-[900] italic absolute top-[14px] left-[165px]"
					style="color:rgb(64 226 255)"
				>
					미션 성공
				</div>
				<div
					class="w-[46px] h-[46px] absolute right-[20px] top-[65px] cursor-pointer"
					style="background-image:url('/img/inGame/clearPop/btn_popup_close.png');"
					on:click={() => (showClearPopup = false)}
				></div>
				<div
					class="w-[1030px] h-[446px] absolute top-[165px] left-[110px]"
					style="background-image:url('/img/inGame/clearPop/ui_clear_background2.png');"
				>
					<div class="absolute w-full top-[55px] left-[-145px]" style="transform:scale(0.7)">
						<div
							class="text-[50px] font-[900] italic absolute top-[50px] left-[50px]"
							style="color:rgb(64 226 255)"
						>
							획득 보상
						</div>
						{#if gameMapDto.rewardExp > 0}
							<div
								id="star1"
								class="w-[203px] h-[203px] absolute top-[175px] left-[50px]"
								style="background-image:url('/img/inGame/clearPop/ui_itemframe.png');transform:scale(1);"
							>
								<div
									class="w-[124px] h-[58px] absolute top-[50px] left-[40px]"
									style="background-image:url('/img/inGame/clearPop/icon_exp.png');transform:scale(1.7);"
								></div>
								<div
									class="text-[60px] text-white font-[900] absolute top-[110px] w-full text-center"
									style="text-shadow:1px 0 black, -1px 0 black, 0 1px black, 0 -1px black;"
								>
									{gameMapDto.rewardExp}
								</div>
							</div>
						{/if}
						{#if gameMapDto.rewardJewel > 0}
							<div
								id="star2"
								class="w-[203px] h-[203px] absolute top-[175px] left-[300px]"
								style="background-image:url('/img/inGame/clearPop/ui_itemframe.png');transform:scale(1);"
							>
								<div
									class="w-[102px] h-[110px] absolute top-[30px] left-[55px]"
									style="background-image:url('/img/inGame/clearPop/icon_gem.png');transform:scale(1.6)"
								></div>
								<div
									class="text-[60px] text-white font-[900] absolute top-[110px] w-full text-center"
									style="text-shadow:"
								>
									{gameMapDto.rewardJewel}
								</div>
							</div>
						{/if}
						{#if gameMapDto.rewardItem}
							<div
								id={gameMapDto.rewardExp == 0 && gameMapDto.rewardJewel == 0 ? 'star1' : 'star3'}
								class="w-[203px] h-[203px] absolute top-[175px] left-[{gameMapDto.rewardExp == 0 &&
								gameMapDto.rewardJewel == 0
									? '50px'
									: '550px'}]"
								style="background-image:url('/img/inGame/clearPop/ui_itemframe.png');transform:scale(1);"
							>
								<div
									class="w-[195px] h-[195px] absolute top-[3px] left-[4px]"
									style="background-image:url('/img/item/{rq.member.player
										.characterType}/{gameMapDto.rewardItem
										?.sourcePath}.png');background-size:contain;background-repeat:no-repeat;"
								></div>
							</div>
						{/if}
					</div>
					<div
						id="star4"
						class="w-[346px] h-[289px] absolute right-[55px] top-[95px]"
						style="background-image:url('/img/inGame/clearPop/icon_complete.png');transform:scale(0.6);"
					></div>
				</div>
				<div
					class="w-[202px] h-[67px] absolute bottom-[60px] left-[110px]"
					style="background-image:url('/img/inGame/clearPop/ui_lv_background.png');"
				>
					<div
						class="font-[900] text-[50px] text-white flex justify-center ml-[85px] leading-[1.2] h-full"
					>
						{rq.getPlayerLeve()}
					</div>
				</div>
				<div
					class="w-[252px] h-[51px] absolute bottom-[75px] left-[400px]"
					style="background-image:url('/img/inGame/clearPop/ui_gembox2.png');transform:scale(1.3);"
				>
					<div
						class="font-[700] text-[40px] text-white flex justify-center ml-[75px] leading-[1.4] h-full"
					>
						{rq.member.player.gems.toLocaleString()}
					</div>
				</div>
				<div
					class="w-[299px] h-[102px] absolute bottom-[50px] right-[85px] cursor-pointer"
					style="background-image:url('/img/inGame/clearPop/btn_action2.png');transform:scale(1.2)"
				>
					<div class="w-[299px] h-[102px] absolute top-[9px] left-[-10px]" style="">
						<div
							class="font-[900] text-[40px] leading-[2.1] italic"
							style="color:rgb(9 13 24);"
							on:click={() => routeToSage()}
						>
							계속
						</div>
					</div>
		...
		<div class="relative bg-gray-500" style="width:{widthMultiplier}px;">
			<div
				class="absolute w-full h-full {showBtnGuide ? 'bg-black bg-opacity-50 z-[99]' : 'hidden'}"
			></div>
			...
			{#if showFailPop}
				...
				>
					<div style="transform:scale({scaleMultiplier2});">
						<div class="mb-[40px]">실패</div>
						<div class="flex flex-row gap-20 text-[50px]">
							<div
								class="cursor-pointer btn btn-lg btn-outline btn-error"
			...
			{#if showSuccessPop}
				...
				>
					<div
						class="flex flex-col gap-4 text-[25px] font-bold"
					...
					>
						...
						<div class="w-full flex justify-end mt-[-60px] ml-[300px]">
							<div
								class="cursor-pointer sc_btn btn btn-lg btn-outline btn-info"
								style="font-size:30px;"
			...
			<div
				class="w-[401px] h-[150px] flex flex-col absolute top-[15%] left-[0] gap-2"
			...
			>
				...
				<div class="flex flex-row absolute top-[72px]">
					<div
						class="w-[76px] h-[36px] ml-2"
						style="background-image:url('/img/inGame/icon_hp.png');transform:scale(0.8)"
					></div>
					<div
						id="health-bar"
						class="flex items-center border-4 w-[260px] ml-4"
						data-total="100"
						data-value="100"
					>
						<div
							class="bar background-bar absolute w-[{hpBackLayer}%]"
							style="width:{hpBackLayer}%"
						></div>
						<div
							class="bar foreground-bar absolute w-[{mainHp}%] {mainHp <= 50
								? mainHp <= 30
									? 'yellow-bar'
									: 'green-bar'
								: ''}"
				...
				{#if usingMedicineStage.includes(gameMapDto.id)}
					<div class="flex flex-row absolute top-[150px]">
						<div
							class="ml-6 w-[80px] h-[80px]"
							style="background-image:url('/img/inGame/medicine.png');background-size:contain;"
						></div>
				...
				{#if usingKitStage.includes(gameMapDto.id)}
					<div class="flex flex-row absolute top-[150px]">
						<div
							class="ml-6 w-[80px] h-[80px]"
							style="background-image:url('/img/inGame/kit.png');background-size:contain;"
						></div>
			...
			<div
				class="w-[1044px] h-[445px] absolute top-[0] right-[0]"
			...
			<!-- mission goal -->
			<div
				class="absolute flex flex-col items-end justify-start absolute right-[1%] top-[2%] text-start"
			...
			>
				...
				<div
					class="flex flex-row gap-2 w-full scale-[0.87] origin-top-right"
					style={showBtnGuide && btnGuideArray[0]
						? 'box-shadow:0 -17px 20px 20px rgb(255, 255, 255, 0.5);'
						: ''}
				>
					<div
						class="w-[506px] h-[134px] flex justify-center items-center italic"
						style="background-image:url('/img/inGame/ui_stage_title.png')"
					>
						<div class="text-[50px] font-[900]" style="color:rgb(64 226 255)">
							{gameMapDto.step}
							{#if gameMapDto.difficulty !== '0'}
								{gameMapDto.difficulty}
							{/if}
						</div>
					</div>
					<div
						class="w-[134px] h-[134px] cursor-pointer btn_setting"
						style="background-image:url('/img/map/btn_settomg_off.png')"
						on:click={() => (openSetting = !openSetting)}
					></div>
				</div>
				<div
					class="flex flex-col"
					style="transform-origin:top right;transform:scale(1.03);
                        {showBtnGuide && btnGuideArray[0]
						? 'box-shadow:0 0 20px 20px rgb(255, 255, 255, 0.5);'
						: ''}"
				>
					<div
						class="w-[547px] h-[76px]"
						style="background-image:url('/img/inGame/ui_goal_start.png');"
					>
						{#if showCompleteBtn}
							<div class="text-[35px] ml-8 font-[900] mt-[15px]" style="color:rgb(255 210 87);">
								목표 : 달성
							</div>
						{:else}
							<div class="text-[35px] ml-8 font-[900] mt-[15px]" style="color:rgb(64 226 255);">
								목표 : 미달성
							</div>
						{/if}
					</div>
					<div
						class="w-[547px] pl-4"
						style="background-image:url('/img/inGame/ui_goal_middle.png');"
					>
			...
			<!-- control box -->
			<div
				class="flex flex-row items-center absolute bottom-[4%] left-[0] gap-6 ml-[2.5%]"
				style="background-color:#181818;transform-origin:left;transform:scale({Math.min(
					1,
					scaleMultiplier
				)});width:{calculateWidthPercentage(Math.min(1, scaleMultiplier))}%;
                {showBtnGuide && btnGuideArray[3]
					? 'z-index:999;box-shadow:0 0 20px 20px rgb(255, 255, 255, 0.5);'
					: ''}"
			>
				<!-- btn Guide 5 -->
				...
				<div class="flex flex-row justify-between h-[52px] w-full">
					<div class="flex flex-row gap-2">
						...
						<div
							class="w-[52px] h-[52px] cursor-pointer"
							style="background-image:{playCanPause
								? 'url("/img/inGame/btn_Control_Pause.png");'
								: isPause
									? 'url("/img/inGame/btn_Control_Play.png");'
									: framesData.length > 0
										? 'url("/img/inGame/btn_Control_Reset.png");'
										: 'url("/img/inGame/btn_Control_Play.png");'}
                            {hasFrameData ? '' : 'pointer-events:none;'}"
							on:click={() => {
								playCanPause ? handlePause() : handlePlay();
							}}
						></div>
					</div>
					<div class="flex justify-end w-full">
						<div class="flex items-center w-[98%]" style="transform:scale(1)">
							...
							<input
								...
								max="0"
								value="0"
								class="w-[98%] range range-xs range-info"
								style="transform-origin:left;"
								bind:this={progressController}
		...
		>
			<div
				id="editor-container-ch1"
				class="w-[633px] h-[1080px] flex flex-col items-center absolute"
				style="background-image:url('/img/inGame/ui_editor_frame.png'), url('/img/inGame/ui_editor_background.jpg');"
			>
				<div
					class="absolute w-full h-full {showBtnGuide ? 'bg-black bg-opacity-50 z-[99]' : 'hidden'}"
				></div>
				<div class="flex flex-row justify-between h-[70px] w-full items-center">
					...
					<!-- editor control -->
					<div
						class="flex flex-row gap-[2rem] mr-10 mt-[-10px] rounded"
						style={showBtnGuide && btnGuideArray[4]
							? 'z-index:999;box-shadow:0 0 20px 20px rgb(255, 255, 255, 0.5);pointer-events:none;'
							: ''}
					>
						<div
							class="w-[34px] h-[34px] z-[50] scale-[0.8] cursor-pointer"
						...
						>
							<div class="absolute text-[35px] top-[630px] left-[90px] ml-[20px] text-white">
								예제코드
							</div>
							<div class="flex flex-col items-center justify-center ml-[73px]">
								<div
									class="font-[900] text-[50px] absolute top-[12px] left-[200px]"
									style="color:rgb(64 226 255)"
								>
									가이드
								</div>
								<div
									class="w-[46px] h-[46px] absolute top-[70px] right-[10px] cursor-pointer"
									style="background-image:url('/img/inGame/btn_popup_close.png')"
									on:click={() => closeModal()}
								></div>
								<div
									class="h-[600px] w-[602px] ml-[50px] text-[25px] mr-[35px] font-bold text-white text-left"
									style="white-space:pre-wrap;"
								>
									<div
										id="typing"
										class="w-[602px] h-[450px] mt-[100px] element"
										style="white-space:pre-wrap;padding:10px 20px;"
									>
										{gameMapDto.guideText}
									</div>
								</div>
								<div
									class="w-[602px] h-[250px] text-[22px] font-bold text-white text-left flex items-center justify-center"
									style="background-image:url('/img/inGame/ui_editor_background4.png');transform:scale(0.8);"
								>
									<div
										class="w-[50px] h-[50px] flex items-center justify-center text-center absolute border-2 rounded-full cursor-pointer top-[25px] right-[40px] hover:scale-110"
										on:click={() => copyText()}
									>
										<i class="fa-solid fa-clone"></i>
									</div>
									<div
										class="stageInfo w-[573px] h-[200px] pl-2 flex items-start justify-start overflow-y-scroll"
										style="white-space:pre-wrap;"
									>
								...
								<div
									class="w-[299px] h-[102px] flex items-center justify-center cursor-pointer"
						...
						<div
							...
							style="background-image:url('/img/inGame/btn_reset.png');transform:scale(0.8);"
						></div>
					</div>
				...
				<div
					id="editorWrapper"
					class="flex justify-center w-[601px] h-[609px] pt-[40px] mt-[4px]"
					style="background-image:url('/img/inGame/ui_editor_background2.png');
                    {showBtnGuide && btnGuideArray[1]
						? 'z-index:999;box-shadow:0 0 20px 20px rgb(255, 255, 255, 0.5);'
						: ''}"
				>
				...
				{#if showCompleteBtn}
					<div
						class="highlighter w-[333px] h-[134px] absolute bottom-[24.5%] left-[51%] z-[10] animatedHighlighter"
						style="pointer-events: none;background-image:url('/img/inventory/ui_aim2.png');background-size:contain;background-repeat:no-repeat;"
					></div>
				{/if}
				...
				<div
					class="flex flex-row justify-around items-center w-[601px] h-[100px] mt-[14px]"
					style="background-image:url('/img/inGame/ui_editor_background3.png');background-size:cover;background-repeat:no-repeat"
				>
					<button
						class="w-[299px] h-[102px] text-[44px] font-[900] italic leading-[2.5]"
						style="background-image:url('/img/inGame/btn_action4.png');color:rgb(9 13 24);transform:scale(0.8);
                            {showBtnGuide && btnGuideArray[5]
							? 'z-index:999;box-shadow:0 0 20px 20px rgb(255, 255, 255, 0.5);background-color:gray;'
							: ''}"
						on:click={() => {
					...
					<button
						class="w-[299px] h-[102px] text-[44px] font-[900] italic leading-[2.5] {showCompleteBtn
							? 'cursor-pointer'
							: 'cursor-default'}"
						style="background-image:{showCompleteBtn
							? 'url("/img/inGame/btn_action2.png");'
							: 'url("/img/inGame/btn_action3.png");'}color:{showCompleteBtn
							? 'rgb(9 13 24)'
							: 'rgb(9 13 24)'};transform:scale(0.8);
                            {showBtnGuide && btnGuideArray[6]
							? 'z-index:999;box-shadow:0 0 20px 20px rgb(255, 255, 255, 0.5);background-image:url("/img/inGame/btn_action2.png");background-color:gray;'
							: ''}"
						on:click={() => {
							showCompleteBtn ? doComplete() : '';
						}}>완료</button
					>
				</div>

				<div
					class="flex justify-start items-center w-[602px] h-[250px] mt-[20px]"
					style="background-image:url('/img/inGame/ui_editor_background4.png');
                    {showBtnGuide && btnGuideArray[2]
						? 'z-index:999;box-shadow:0 0 20px 20px rgb(255, 255, 255, 0.5);'
						: ''}"
				>
					<div
						class="command-guide w-[590px] h-[210px] flex flex-col items-start pl-10 gap-2 pt-2 overflow-y-scroll"
					>
						{#each commandGuide as command}
							{#if command}
								<div
									class="scale-up-on-hover font-bold text-white text-[22px] font-[900] cursor-pointer"
									on:click={() => appendCodeToEditor(command)}
								>
									{command}
								</div>
							{/if}
