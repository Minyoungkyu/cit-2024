...
<div class="w-full h-full flex flex-col">
    <div class="flex flex-row w-full my-4 justify-around">
        <div class="flex flex-row gap-4">
            <button class="btn btn-sm" on:click={() => window.location.href="/adm/menu/member/class/new"}>생성</button>
            <button class="btn btn-sm" on:click={() => handleCheckedPrograms()}>삭제</button>
            <!-- <button class="btn btn-sm">엑셀 다운로드</button> -->
            <a href="{import.meta.env.VITE_CORE_API_BASE_URL}/api/v1/members/class/download/csv" class="btn btn-sm">엑셀 다운로드</a>
        </div>
        ...
        <table cellpadding="15" cellspacing="15" width="80%" class="mx-auto">
            ...
            <tbody>
                ...
                <tr class="text-center whitespace-nowrap border-b border-gray-200 text-sm lg:text-md" >
                    ...
                    <td >{truncateProgramName(member.schoolName)}</td>
                    <td >{member.createDate}</td>
                    <td>
                        <a href="/adm/menu/member/class/{member.id}" class="btn btn-xs">수정</a>
                    </td>
                </tr>
                {/each}
            </tbody>
