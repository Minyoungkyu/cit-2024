<script lang="ts">
	...
	const {
		...
		stepsLevelCount,
		playerLogList,
		difficultySelectorMsg,
		difficultySelectorName,
		activeTransitionAnimation
	} = $props<{
		widthValue: Number;
		scaleMultiplier: Number;
		gameMapId: number;
		stepsLevelCount: number;
		playerLogList: components['schemas']['PlayerLogDto'][];
		difficultySelectorMsg: string;
		difficultySelectorName: string;
		activeTransitionAnimation: () => void;
	}>();
	...
	rq.unlock().then((res) => {
		ress = res;
		isUnLockEasy = ress!.some(
			(log: number) => log >= gameMapId && log < gameMapId + stepsLevelCount
		);
		isUnLockNormal = ress!.some(
			(log: number) => log >= normalDifficultyThreshold + 1 && log < hardDifficultyThreshold + 1
		);
		isUnLockHard = ress!.some(
			(log: number) =>
				log >= hardDifficultyThreshold + 1 && log < hardDifficultyThreshold + 1 + stepsLevelCount
		);
	});

	const hasClearedEasy = playerLogList
		.filter((log) => log.detailInt! >= 1)
		.some((log) => easyDifficultyThreshold.includes(log.gameMapId));
	const hasClearedNormal = playerLogList
		.filter((log) => log.detailInt! >= 1)
		.some(
			(log) => log.gameMapId >= normalDifficultyThreshold && log.gameMapId < hardDifficultyThreshold
		);
	const hasClearedHard = playerLogList
		.filter((log) => log.detailInt! >= 1)
		.some(
			(log) =>
				log.gameMapId >= hardDifficultyThreshold &&
				log.gameMapId < hardDifficultyThreshold + stepsLevelCount
		);
	...
	let difficultiesGameMapId: number[] = [
		gameMapId,
		gameMapId + stepsLevelCount,
		gameMapId + 2 * stepsLevelCount
	]; // 각 난이도 첫 맵 아이디
	...
	async function routePlayerToLastGame() {
		const { data } = await rq
			...
			.GET(`/api/v1/playerLogs/gamesLastLog/{gameMapId}`, {
				params: {
					path: {
...
<div
	class="flex dropdown-content justify-end pt-12 gap-12 h-[1080px] w-[575px] absolute right-[0] slide-in z-[98]"
...
>
	<div class="flex flex-col w-[501px]">
		<div class="flex justify-end w-full mr-16">
			<div
				class="text-[70px] font-extrabold text-white text-right mr-[50px]"
				style="text-shadow:6px 6px #666"
			>
				{difficultySelectorName}
			</div>
		</div>
		<div class="flex flex-col items-end w-full">
			<div
				class="w-[501px] h-[52px]"
				style="background-image:url('/img/map/ui_mission_top.png')"
			></div>
		...
		<div class="flex flex-col gap-2 items-center p-2">
			<div class="flex flex-row items-center gap-4">
				<button
					id="decrease"
					on:click={decreaseDifficulty}
					class="w-[45px] h-[63px]"
					style="background-image:url('/img/map/btn_next3.png')"
				></button>
				<div
					id="difficulty"
					class="leading-[40px] px-8 w-[314px] h-[76px] text-center font-bold text-white text-[40px] leading-[70px]"
					style="background-image:url('/img/map/ui_stage_Level.png')"
				>
					{difficulties[currentIndex]}
				</div>
				<button
					id="increase"
					on:click={increaseDifficulty}
					class="w-[45px] h-[63px]"
					style="background-image:url('/img/map/btn_next4.png')"
				></button>
			</div>
		</div>
		...
		{:else}
			<div class="flex gap-2 justify-center w-[501px] p-2">
				<button
					class="w-[333px] h-[116px]"
					style="background-image:url('/img/map/btn_action.png')"
...
{#if $showCharacterStatusModal}
	<CharacterStatusModal
		{widthValue}
		{scaleMultiplier}
		gameMapDto={routeGameMapDto}
		requiredPartsList={routeGameRequiredPartsList}
		{activeTransitionAnimation}
	/>
{/if}
